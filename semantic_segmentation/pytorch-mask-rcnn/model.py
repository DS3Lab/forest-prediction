...
class ResNet(nn.Module):
    ...
    def __init__(self, architecture, stage5=False):
        ...
        self.C1 = nn.Sequential(
            ...
            nn.Conv2d(6, 64, kernel_size=7, stride=2, padding=3), # 6 channels = 2 RGBS
...
def detection_layer(config, rois, mrcnn_class, mrcnn_bbox, image_meta):
    ...
    _, window, _ = parse_image_meta(image_meta)
    window = window[0]
...
class MaskRCNN(nn.Module):
    ...
    def train_epoch(self, datagenerator, optimizer, steps):
        ...
        optimizer.zero_grad()
        # for i, inputs in enumerate(tqdm(datagenerator, total=len(datagenerator))):
        for i, inputs in enumerate(datagenerator):
            print('Image', i)
            batch_count += 1
...
def parse_image_meta(meta):
    ...
    return image_shape, window, active_class_ids
