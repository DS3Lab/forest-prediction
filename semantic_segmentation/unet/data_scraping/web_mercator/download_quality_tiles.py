...
def download_item(tile, z, out_dir):
    ...
    Download quad tif
    """
    forest_loss_path2013 = os.path.join(out_dir, 'forest_loss', 2013, forest_loss_name.format(year=2013, z=z, x=tile[0], y=tile[1]))
    forest_loss_url2013 = FOREST_LOSS_URLS[str(2013)].format(z=z, x=x, y=y)
    response2013 = download_file(forest_loss_url, forest_loss_path, redo=False)
    if response2013 == 1:
        ...
        if check_quality_label:
            ...
            logger.debug('Not quality:' + forest_loss_url2013)
    else:
        logger_fail.debug('File not downloaded:' + forest_loss_url2013)

def download_others(tile, z, out_dir):
