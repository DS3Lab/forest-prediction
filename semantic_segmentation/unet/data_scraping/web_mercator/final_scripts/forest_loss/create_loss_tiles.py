...
def get_file(files, year, zoom, x, y):
    template = 'ly{year}_{zoom}_{x}_{y}.png'
    fs = template.format(year=year, zoom=zoom, x=x, y=y)
    for file in files:
        f = file.split('/')[-1]
...
def create_loss_tiles(hansen_files, other_files_dir, out_dir):
    template = 'ly{year}_{zoom}_{x}_{y}.png'
    out_file_temp = 'ly{year}_{zoom}_{x}_{y}.npy'
    for file in hansen_files:
        year, zoom, x, y = get_tile_info(file)
        py_file = template.format(year=int(year)-1, zoom=zoom, x=x, y=y)
        tmp_file = os.path.join(other_files_dir, py_file) # previous year file
        if os.path.exists(tmp_file):
            py_file = tmp_file
        else:
            py_file = get_file(hansen_files, int(year)-1, zoom, x, y)
            if not py_file:
...
def main():
    ...
    ########### SAVE LOSS TILES ###############
    create_loss_tiles(hansen_files, out_hansen_dir, out_hansen_loss_dir)
